...
function buildTrack(){
 ...
 horses.forEach((h,i)=>{
  ...
  e.style.top=(8+i*12)+"%"; e.style.left="0%";
 });
}
buildTrack();
...
$("startBtn").onclick=()=>{
 ...
 horses.forEach((h,i)=>{horseEls[h.no].style.left="0%";horseEls[h.no].style.top=(8+i*12)+"%"});
...
function frame(now){
 ...
 horses.forEach((h,i)=>{
  ...
  e.style.left=(p*100)+"%";
  e.style.top=(8+i*12 + Math.sin(t*8+i)*0.7)+"%";
 });
...
function finishRace(){
 ...
 results=ranked;
 ranked.forEach((h,i)=>{horseEls[h.no].style.left="99%";horseEls[h.no].style.top=(8+(i%8)*12)+"%"});
